...
EXCLUDE_NAMES = {"USDTRY", "USDTRY=X", "ISKUR"}  # ISKUR: veri hatasi, tek gunde %2M+ getiri (bkz. find_extreme_return.py)
MAX_PLAUSIBLE_DAILY_RETURN = 1.0  # %100 tek gunluk getiri ustu = veri hatasi supheli, otomatik ele
IPO_SEASONING_DAYS = 60  # her sembolun veri setindeki ilk N islem gunu, "yeni halka arz" donemi sayilip olay taramasindan haric tutulur


def load_market_index(path):
    """Gercek XU100 endeks getirisini yukler. Bulunamazsa None doner ve
    caller esit-agirlikli fallback proxy'ye geri duser (uyari ile)."""
...
def main():
    ...
    # Guvenlik filtresi: tek gunde %100+ getiri gosteren sembolleri otomatik
    # tespit et ve panelden cikar. Boyle bir hareket neredeyse her zaman veri
    # hatasidir (ISKUR gibi - bkz. find_extreme_return.py) ve elenmezse
    # tum tertile ortalamalarini anlamsiz hale getirir.
    max_abs_ret = ret_panel.abs().max(axis=0)
    suspect_symbols = max_abs_ret[max_abs_ret > MAX_PLAUSIBLE_DAILY_RETURN].index.tolist()
    if suspect_symbols:
        print(f"UYARI: {len(suspect_symbols)} sembolde imkansiz gorunen tek-gunluk "
              f"getiri (>{int(MAX_PLAUSIBLE_DAILY_RETURN*100)}%) tespit edildi, "
              f"otomatik cikariliyor: {suspect_symbols}")
    ...
    if xu100_ret is not None:
        ...
        # XU100'de olup hisse panelinde olmayan gunler NaN kalabilir; forward_excess
        # zaten mkt_fwd hesaplarken bu tarihleri kullanacak, eksikse o event atlanir.
        market_source = f"XU100 gercek endeks ({MARKET_INDEX_PATH})"
    else:
        market_ret = ret_panel.mean(axis=1, skipna=True)  # fallback: esit-agirlikli proxy
        market_source = "ESIT-AGIRLIKLI PROXY (XU100 bulunamadi, fallback kullanildi!)"
    ...
    # IPO/yeni-listing sezonlama filtresi: her sembolun kendi veri setindeki ilk
    # gecerli gununden itibaren IPO_SEASONING_DAYS islem gunu boyunca hicbir
    # olay sayilmaz. Gerekce: taze halka arzlar ilk haftalarda/aylarda fiyat
    # kesfi nedeniyle asiri oynak olur (bkz. BALAT 2012-08 halka arz -> 2012-09
    # cokusu outlier_events.csv'de) - bu genel bir "extreme_down_reversal"
    # davranisi degil, IPO'ya ozgu ayri bir fenomendir.
    seasoning_cutoff = {}
    ...
    events_extreme = []
    excluded_seasoning_volspike = 0
    excluded_seasoning_extreme = 0
    ...
    if suspect_symbols:
        report_lines.append(f"Auto-excluded (implausible daily return >{int(MAX_PLAUSIBLE_DAILY_RETURN*100)}%): {suspect_symbols}")
    report_lines.append(f"Market benchmark: {market_source}")
    report_lines.append(f"IPO seasoning filter: ilk {IPO_SEASONING_DAYS} islem gunu haric tutuldu "
                         f"(elenen: {excluded_seasoning_volspike} volume_spike, "
                         f"{excluded_seasoning_extreme} extreme_down adayi)")

    ...
    for horizon in horizons:
        ...
        for direction, shock_dates in [('TL_WEAKENS', shock_up_dates), ('TL_STRENGTHENS', shock_down_dates)]:
            ...
            for d in valid_dates:
                ...
                all_returns.extend(row.tolist())

            if len(all_returns) < 5:
                continue

            all_returns = np.array(all_returns) * 10000
            mean_bps = all_returns.mean()
            t_stat, p_value = ttest_1samp(all_returns, 0)
...
def test_fx_momentum_correlation(stock_returns, usdtry_returns, lookbacks, horizons, date_filter=None):
    results = []
    ...
    for lookback in lookbacks:
        ...
        for horizon in horizons:
            market_avg_fwd = stock_returns.shift(-1).rolling(horizon).sum().shift(-(horizon - 1)).mean(axis=1)
...
def main():
    data_dir = 'data'
    stock_close = read_stock_data(data_dir)
    ...
    for period_name, date_filter in [('full', None), ('older', (start_date, mid_date)),
        ...
        res = test_fx_shock_reaction(stock_returns, usdtry_returns, shock_threshold_pct=0.90,
    ...
    for period_name, date_filter in [('full', None), ('older', (start_date, mid_date)),
                                      ('recent', (mid_date, end_date))]:
        res = test_fx_momentum_correlation(stock_returns, usdtry_returns, lookbacks, horizons,

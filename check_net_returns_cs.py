...
COMMISSION_BPS = 38.0  # AtaYatirim round-trip, check_net_returns.py COMMISSION_SCENARIOS_BPS ile ayni gercek tarife
...
def corwin_schultz_spread(high, low):
    ...
    alpha = (np.sqrt(2) - 1) * sqrt_beta / K - sqrt_gamma / np.sqrt(K)
...
def main():
    ...
    def forward_net_profit(events_df, horizon):
        ...
        for name, d in zip(events_df["symbol"], events_df["date"]):
            ...
            round_trip_cost = spread_estimate + COMMISSION_BPS / 10000.0  # spread + komisyon
